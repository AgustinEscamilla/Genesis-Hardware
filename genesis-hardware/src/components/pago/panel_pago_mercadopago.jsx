...
export function PanelPagoMercadoPago({ total, alExito, alVolver }) {
  const { estado, error, alEnviar } = usePagoMercadopago(alExito)

  return (
    <div className="flex flex-col gap-3">
      <TarjetaResumenPago total={total} />
      {estado !== 'procesando' && <BrickPagoMercadoPago total={total} alEnviar={alEnviar} />}
      {estado === 'procesando' && <p className="text-xs text-terciario">Procesando pago de prueba...</p>}
