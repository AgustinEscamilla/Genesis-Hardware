...
export function FormularioEdicionCatalogo({ seleccionado, alGuardar, urlImagen, subiendo, alCambiarArchivo }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); alGuardar({ ...forma, imagen: urlImagen, stockVisible: Number(forma.stockVisible || 0) }) }} className="flex flex-col gap-3">
      ...
      <button type="submit" disabled={subiendo} className="border border-primario text-primario text-xs px-4 py-2 hover:bg-primario hover:text-fondo disabled:opacity-50 transition-colors">
        {seleccionado ? 'Actualizar producto' : 'Agregar producto'}
      </button>
    </form>
  )
}
