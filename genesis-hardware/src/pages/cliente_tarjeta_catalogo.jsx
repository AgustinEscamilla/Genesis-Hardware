...
export function ClienteTarjetaCatalogo({ producto, alAgregar }) {
  ...
  return (
    <div className="border border-borde bg-panel p-3 flex flex-col gap-3">
      <p className="text-[10px] text-mutado">SKU {producto.id?.slice(0, 8)}</p>
      <p className={`text-xs ${critico ? 'text-primario' : 'text-[#d8b4fe]'}`}>{critico ? 'LOW STOCK' : 'IN STOCK'}</p>
      ...
      <div className="mt-auto flex justify-between items-center pt-2 border-t border-borde">
        ...
        <button onClick={() => alAgregar(producto)} className="border border-[#d8b4fe] text-[#d8b4fe] px-4 py-1 text-xs hover:bg-[#d8b4fe] hover:text-fondo">ADD</button>
      </div>
    </div>
  )
}
