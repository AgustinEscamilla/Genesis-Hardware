...
// aqui maestro yo muestro los pedidos recien pagados listos para empacar o asignar
export function VistaPedidosPagadosEmpleado() {
  const { pedidos, cargando, error } = usePedidosPagadosEmpleado()

  if (cargando) return <div className="flex min-h-40 items-center justify-center text-xs text-slate-400">Cargando pedidos pagados</div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100 flex flex-col gap-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-lg">
        <h2 className="text-xl font-bold mb-1">Pedidos pagados</h2>
        <p className="text-xs text-slate-400">Empaca los pedidos recien pagados y asignalos al repartidor</p>
      </div>
