...
export function SeccionModulos() {
  ...
  return (
    <section id="noticias" className="mx-auto max-w-7xl bg-fondo px-4 py-20 md:px-8">
      ...
      <div className="overflow-x-auto rounded-xl border border-borde bg-panel shadow-vidrio">
        <table className="min-w-[720px] w-full text-left text-xs">
          ...
          <tbody>
            {noticias.map((noticia) => <tr key={noticia.id} className="border-b border-borde/70 last:border-0"><td className="whitespace-nowrap px-4 py-4 text-mutado">Reciente</td><td className="px-4 py-4"><p className="font-bold text-texto">{noticia.titulo}</p><p className="mt-1 text-[10px] text-primario">{noticia.etiqueta || 'Actualizacion'}</p></td><td className="max-w-md px-4 py-4 leading-5 text-mutado">{noticia.descripcion}</td><td className="px-4 py-4"><span className="rounded-full border border-terciario/30 bg-terciario/10 px-2 py-1 text-[10px] font-bold text-terciario">{noticia.valor || 'Publicado'}</span></td></tr>)}
