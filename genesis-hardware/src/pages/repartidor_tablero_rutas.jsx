  ...
  return (
    <section className="flex-1 p-4 flex flex-col gap-4">
      <div className="flex justify-between items-center border-b border-borde pb-3">
        <div><p className="text-xs text-[#d8b4fe]">DESPACHO EN VIVO</p><h1 className="text-5xl font-black">Panel de Reparto y Rutas</h1></div>
        <div className="flex gap-2"><button className="border border-borde px-4 py-2 text-xs">Sincronizar GPS</button><button className="border border-[#d8b4fe] bg-[#d8b4fe] text-fondo px-4 py-2 text-xs">Escanear paquete</button></div>
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <div className="border border-borde bg-panel p-3"><p className="text-xs text-mutado mb-3">SECUENCIA DE MANIFIESTO</p>{!pedidos.length && <p className="text-xs text-mutado">Sin pedidos listos</p>}{pedidos.map(p => <p key={p.id} className="text-xs text-texto mb-2">Pedido {p.id.slice(0, 8)}</p>)}</div>
